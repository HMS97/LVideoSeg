...
class RecurrentMemoryTransformer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        b, n, device, mem_length, return_loss = *x.shape[:2], x.device, self.num_memory_tokens, exists(labels)

        assert n <= self.seq_len

        pos = torch.arange(n, device = device)
        
        # print('before RMT embedding', x.shape)
        # x = self.token_emb(x)
        # print('RMT embedding', x.shape)
